...
# 22. calendar_sources — externe Stundenplan-/Kalenderquellen (UP-8, Schritt 1)
class CalendarSource(Base):
    """Eine eingerichtete Quelle. Beim WebUntis-Weg genügt **eine** schulweite Zeile.

    `pseudonym = NULL` heißt schulweit. Die Spalte existiert für den ICS-Rückfallweg, bei
    dem jede Lehrkraft ihre eigene Abo-URL hätte; beim Servicekonto bleibt sie leer.

    Zugangsdaten stehen verschlüsselt in `config` (`app/calendar/secrets.py`), niemals im
    Klartext — die Spalte landet in jedem Backup.
    """
    ...
    # Kurzfassung für die Statusanzeige — OHNE Zugangsdaten. Siehe CalendarSourceError.
    ...
    __table_args__ = (
        ...
        # Je Art und Inhaber höchstens eine Quelle. Zwei aktive Quellen derselben Art
        # würden einander überschreiben, ohne dass jemand sähe, welche gewonnen hat.
        Index(
        ...
        Index(
            ...
            "kind",
            unique=True,
            postgresql_where=text("pseudonym IS NULL"),
        ),
    )
