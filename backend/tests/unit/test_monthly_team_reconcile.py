...
from scripts.monthly_team_reconcile import run
...
async def test_run_happy_path_single_student():
    """1 Student, grade=7 -> Team-Abgleich läuft, Budget bleibt unberührt."""
    user = _make_user(pseudonym="p1", role="student", grade=7)
    session_factory = _make_db_session([user])

    with patch("scripts.monthly_team_reconcile.AsyncSessionLocal", return_value=session_factory), \
         patch("scripts.monthly_team_reconcile.LiteLLMClient") as mock_client_cls, \
         patch("scripts.monthly_team_reconcile.reconcile_user_team", new=AsyncMock(return_value={"unchanged": True, "added": [], "removed": []})), \
         patch("scripts.monthly_team_reconcile._extract_current_team_ids", return_value=set()):
...
async def test_run_dry_run_skips_api_calls():
    ...
    with patch("scripts.monthly_team_reconcile.AsyncSessionLocal", return_value=session_factory), \
         patch("scripts.monthly_team_reconcile.LiteLLMClient") as mock_client_cls, \
         patch("scripts.monthly_team_reconcile.reconcile_user_team") as mock_reconcile, \
         patch("scripts.monthly_team_reconcile._extract_current_team_ids", return_value=set()):
...
async def test_run_invalid_grade_skips_team_but_updates_budget():
    """grade=99 -> get_target_team_id wirft ValueError -> Team-Phase wird übersprungen."""
...
async def test_run_teacher_uses_lehrkraefte_team():
    ...
    with patch("scripts.monthly_team_reconcile.AsyncSessionLocal", return_value=session_factory), \
         patch("scripts.monthly_team_reconcile.LiteLLMClient") as mock_client_cls, \
         patch("scripts.monthly_team_reconcile.reconcile_user_team", new=mock_reconcile):
