<script>
  ...
  $effect(() => {
    ...
    getContextNode(id)
      .then(n => {
        ...
        if (n.category === 'concept' && n.content_type === 'funktion' && n.metadata?.signatur) {
          signatur = n.metadata.signatur
        } else if (n.category === 'concept' && n.content_type === 'bauteil' && n.metadata?.schaltzeichen) {
  ...
  async function save() {
    ...
    try {
      ...
      setTimeout(() => { savedMessage = null }, 2000)
    } catch (e) {
      if (e.status === 422) {
  ...
  {:else}
    <div class="mb-4">
      <div class="flex items-center gap-3">
        <h1 class="text-2xl font-bold text-light-tx dark:text-dark-tx">Knoten bearbeiten</h1>
        {#if node}
