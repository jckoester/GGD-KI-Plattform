<script>
    ...
    // „Zurück"/„Abbrechen" führen zur Leseansicht des Knotens; der ursprüngliche
    // back-Parameter (z. B. die Liste) wird durchgereicht.
    const readUrl = $derived(
        `/knowledge/${$page.params.id}` +
            (backParam ? `?back=${encodeURIComponent(backParam)}` : ""),
    );
            ...
            .then((n) => {
                ...
                const editable =
                    ...
                    (($user.roles?.includes("admin") ?? false) ||
                        n.owner_pseudonym === $user.pseudonym);
                if ($user && !editable) {
                    goto(
                        `/knowledge/${id}` +
                            (backParam
    ...
    const contentTypeOptions = $derived(
        auswaehlbareTypOptionen(category ? (CONTENT_TYPES[category] ?? []) : [], contentType),
    );
    ...
    async function save() {
        ...
        try {
            ...
            // Zurück zur Leseansicht (zeigt das gerenderte Ergebnis)
            goto(readUrl);
        } catch (e) {
            if (e.status === 422) {
                errors.general = e.message;
...
<div class="h-full overflow-y-auto p-6 max-w-2xl">
    ...
    >
        <ArrowLeft class="w-4 h-4" /> Zurück zur Ansicht
    </a>
    {#if loadingNode}
        <div
