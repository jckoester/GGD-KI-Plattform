<script>
    ...
    function getActiveTrigger(text, cursorPos) {
        ...
        if (lpMatch) return { kind: 'lp', query: lpMatch[1], matchStart: before.length - lpMatch[0].length }
    ...
    async function search(trigger) {
        if (trigger.kind === 'lp') {
            loading = true
            try {
                const params = new URLSearchParams({ content_type: 'leitperspektive', limit: '10' })
...
<div class="space-y-2">
    ...
    <div class="relative">
        <textarea
            ...
            rows="2"
            placeholder="Hinweise: @LP für Leitperspektive, #ETH 3.1 für Fachbezug, freier Text für MINT etc."
        ...
        <div class="flex flex-wrap gap-1 items-center text-xs text-light-tx-2 dark:text-dark-tx-2">
            ...
            {#each previewParts as part}
                ...
                {:else if part.label.trim()}
                    <span class="text-light-tx dark:text-dark-tx">{part.label}</span>
                {/if}
            {/each}
        </div>
    {/if}
