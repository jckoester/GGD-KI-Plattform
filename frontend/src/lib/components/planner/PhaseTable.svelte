<script>
    ...
    const gesamtMin = $derived(
        (phasen || []).reduce((s, p) => s + (p.dauer_min || 0), 0),
    );
    ...
    const prioSegments = $derived(() => {
        ...
        for (const p of phasen || []) {
            totals[p.prio || "kern"] =
                (totals[p.prio || "kern"] || 0) + (p.dauer_min || 0);
        }
        return totals;
    });
...
<div class="space-y-2">
    ...
    <div>
        <div class="flex items-baseline gap-2 mb-1">
            ...
            {#if ueberhang > 0}
                <span
                    class="text-xs font-semibold text-light-re dark:text-dark-re"
                >
                    +{ueberhang}′ Überhang
                </span>
            {/if}
        ...
        <div
            class="h-2.5 rounded-full overflow-hidden flex bg-light-ui-2 dark:bg-dark-ui-2 relative"
        >
            {#if gesamtMin > 0}
                ...
                {#each ["kern", "uebung", "vertiefung"] as prio}
                    ...
                    {#if min > 0}
                        <div
                            ...
                            style="width: {(min / ref) *
                                100}%; background-color: {PRIO_COLORS[prio][0]}"
                            title="{prio}: {min}′"
                        ></div>
                    {/if}
                {/each}
    ...
    {#if phasen.length > 0}
        <div class="overflow-x-auto">
            <table class="w-full text-sm border-collapse">
                <thead>
                    <tr
                        class="border-b-2 border-light-ui-3 dark:border-dark-ui-3
