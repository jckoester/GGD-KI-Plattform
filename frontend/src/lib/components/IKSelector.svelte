<script>
    ...
    function addByNumber() {
        ...
        const found = searchResults.find(r => extractNrFromTitle(r.title) === nr)
        if (found) {
            selectIK(found)
        } else {
...
<div class="space-y-2">
    ...
    {#if selected.length > 0}
        <div class="flex flex-wrap gap-2">
            {#each selected as ik}
                <div class="flex items-center gap-1 px-2 py-1 rounded-full bg-light-bl/20 dark:bg-dark-bl/20">
                    <input
                        type="checkbox"
                        checked={ik.partiell}
        ...
        {#if showDropdown && (searchResults.length > 0 || loading)}
            <div class="absolute top-full left-0 right-0 mt-1 z-50 bg-white dark:bg-dark-bg-2 
                ...
                {:else if searchResults.length === 0}
                    <div class="p-3 text-sm text-light-tx-2 dark:text-dark-tx-2">
                        Keine IK-Kompetenz gefunden. Drücken Sie Enter, um die Nummer trotzdem hinzuzufügen.
                    </div>
                {:else}
                    {#each searchResults as node (node.id)}
                        ...
                        >
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="text-xs text-light-tx-2 dark:text-dark-tx-2 truncate">
