<script>
    ...
    function addLernsequenz(kapitelIndex) {
        if (!curriculum || !curriculum.kapitel || kapitelIndex >= curriculum.kapitel.length) {
            return
        }
    ...
    async function executeDelete() {
        ...
        if (type === 'kapitel') {
            ...
            if (kapitel) {
                kapitel.lernsequenzen = kapitel.lernsequenzen.filter(ls => ls.id !== id)
                curriculum.kapitel = [...curriculum.kapitel]
            }
            ...
            if (kapitel) {
                ...
                if (ls && ls.metadata?.eintraege) {
                    ls.metadata.eintraege = ls.metadata.eintraege.filter(e => e !== id)
                    curriculum.kapitel = [...curriculum.kapitel]
                }
            }
        }
     ...
     */
    function handleDrop(event, type, id, chapterId = null, lsId = null, dropPosition = 'before') {
        event.preventDefault()
        ...
        if (type === 'kapitel' && draggedItem.type === 'kapitel') {
            // Kapitel verschieben
            const fromIndex = curriculum.kapitel.findIndex(k => k.id === draggedItem.id)
            const toIndex = curriculum.kapitel.findIndex(k => k.id === id)
            if (fromIndex !== -1 && toIndex !== -1) {
                const [moved] = curriculum.kapitel.splice(fromIndex, 1)
                curriculum.kapitel.splice(dropPosition === 'before' ? toIndex : toIndex + 1, 0, moved)
        ...
        } else if (type === 'lernsequenz' && draggedItem.type === 'lernsequenz' && chapterId) {
            // Lernsequenz innerhalb eines Kapitels verschieben
            const kapitel = curriculum.kapitel.find(k => k.id === chapterId)
            if (kapitel) {
                const fromIndex = kapitel.lernsequenzen.findIndex(ls => ls.id === draggedItem.id)
                const toIndex = kapitel.lernsequenzen.findIndex(ls => ls.id === id)
                if (fromIndex !== -1 && toIndex !== -1) {
                    const [moved] = kapitel.lernsequenzen.splice(fromIndex, 1)
                    kapitel.lernsequenzen.splice(dropPosition === 'before' ? toIndex : toIndex + 1, 0, moved)
                    // Reihenfolge aktualisieren
        ...
        } else if (type === 'eintrag' && draggedItem.type === 'eintrag' && chapterId && lsId) {
            // Eintrag innerhalb einer Lernsequenz verschieben
            const kapitel = curriculum.kapitel.find(k => k.id === chapterId)
            if (kapitel) {
                const ls = kapitel.lernsequenzen.find(ls => ls.id === lsId)
                if (ls && ls.metadata?.eintraege) {
                    const fromIndex = ls.metadata.eintraege.findIndex(e => e === draggedItem.id)
                    const toIndex = ls.metadata.eintraege.findIndex(e => e === id)
                    if (fromIndex !== -1 && toIndex !== -1) {
                        const [moved] = ls.metadata.eintraege.splice(fromIndex, 1)
                        ls.metadata.eintraege.splice(dropPosition === 'before' ? toIndex : toIndex + 1, 0, moved)
                        curriculum.kapitel = [...curriculum.kapitel]
                        onchange()
                    }
                }
...
{#if showContextMenu}
    ...
    >
        <button
            onclick={() => {
                confirmDelete(showContextMenu.type, showContextMenu.id, 
                    showContextMenu.type === 'kapitel' ? curriculum.kapitel.find(k => k.id === showContextMenu.id)?.title : 
                    showContextMenu.type === 'lernsequenz' ? curriculum.kapitel.find(k => k.id === showContextMenu.chapterId)?.lernsequenzen.find(ls => ls.id === showContextMenu.id)?.title :
                    'Eintrag',
                    showContextMenu.chapterId, showContextMenu.lsId)
                hideMenu()
            }}
...
{#if deleteConfirm}
    <div class="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-dark-bg-2 rounded-lg border border-light-ui-3 dark:border-dark-ui-3 p-6 max-w-md w-full" onclick={(e) => e.stopPropagation()}>
            <h3 class="text-lg font-semibold text-light-tx dark:text-dark-tx mb-3">
                Löschen bestätigen
            </h3>
            <p class="text-light-tx-2 dark:text-dark-tx-2 mb-4">
                Möchten Sie "{deleteConfirm.title}" wirklich löschen? Dieser Vorgang kann nicht rückgängig gemacht werden.
            </p>
    ...
    <table class="w-full text-left border-collapse text-sm">
        ...
        <tbody>
            {#each curriculum?.kapitel || [] as kap, kapIndex (kap.id)}
                <!-- Kapitel-Kopfzeile -->
                <tr 
                    class="bg-light-bg-2 dark:bg-dark-bg-2 {editMode ? 'cursor-move' : ''}"
                    draggable={editMode}
                    ondragstart={(e) => { e.preventDefault(); handleDragStart(e, 'kapitel', kap.id) }}
                    ondragover={(e) => { e.preventDefault(); handleDragOver(e) }}
                    ondrop={(e) => { e.preventDefault(); handleDrop(e, 'kapitel', kap.id) }}
                >
                    <td 
                    ...
                    >
                        {#if editMode}
                            <div class="flex items-center gap-2">
                                <Move class="w-4 h-4 text-light-tx-2 dark:text-dark-tx-2 cursor-move" />
                                <input
                                    type="text"
                                    value={kap.title}
                                    oninput={(e) => {
                                ...
                                >
                                    <MoreVertical class="w-4 h-4 text-light-tx-2 dark:text-dark-tx-2" />
                                </button>
                            </div>
                        {:else}
                            {kap.title}
                        ...
                        >
                            {#if editMode}
                                <textarea
                                    ...
                                    oninput={(e) => {
                                        kap.metadata = { ...kap.metadata, einleitung: e.target.value }
                                        curriculum.kapitel = [...curriculum.kapitel]
                                        onchange()
                                    }}
                ...
                {#each kap.lernsequenzen || [] as ls, lsIndex (ls.id)}
                    ...
                    {#if ls.title || editMode}
                        <tr 
                            class="bg-light-bg-2 dark:bg-dark-bg-2 {editMode ? 'cursor-move' : ''}"
                            draggable={editMode}
                            ondragstart={(e) => { e.preventDefault(); handleDragStart(e, 'lernsequenz', ls.id, kap.id) }}
                            ondragover={(e) => { e.preventDefault(); handleDragOver(e) }}
                            ondrop={(e) => { e.preventDefault(); handleDrop(e, 'lernsequenz', ls.id, kap.id) }}
                        >
                            <td 
                                colspan="{editMode ? 5 : 4}" 
                            ...
                            >
                                {#if editMode}
                                    <div class="flex items-center gap-2">
                                        ...
                                        {/if}
                                        <button
                                            onclick={(e) => showMenu(e, 'lernsequenz', ls.id, kap.id)}
                                            class="p-1 hover:bg-light-ui-2 dark:hover:bg-dark-ui-2 rounded"
                                        >
                                            <MoreVertical class="w-4 h-4 text-light-tx-2 dark:text-dark-tx-2" />
                                        </button>
                    ...
                    {#each ls.metadata?.eintraege || [] as eintrag, entryIndex (entryIndex)}
                        <tr 
                            class="border-b border-light-ui-3 dark:border-dark-ui-3 
                                   {entryIndex % 2 === 0 ? 'bg-white dark:bg-transparent' : 'bg-light-bg-2/50 dark:bg-dark-bg-2/50'}"
                            draggable={editMode}
                            ondragstart={(e) => { e.preventDefault(); handleDragStart(e, 'eintrag', entryIndex, kap.id, ls.id) }}
                            ondragover={(e) => { e.preventDefault(); handleDragOver(e) }}
                            ondrop={(e) => { e.preventDefault(); handleDrop(e, 'eintrag', entryIndex, kap.id, ls.id) }}
                        >
                            ...
                            <td class="px-3 py-2 vertical-align-top">
                                {#if editMode}
                                    <textarea
                                        ...
                                        oninput={(e) => {
                                            eintrag.konkretisierung = e.target.value
                                            curriculum.kapitel = [...curriculum.kapitel]
                                            onchange()
                                        }}
                            ...
                            {#if editMode}
                                <td class="px-3 py-2 text-center">
                                    <button
                                        onclick={(e) => showMenu(e, 'eintrag', entryIndex, kap.id, ls.id)}
                                        class="p-1 hover:bg-light-ui-2 dark:hover:bg-dark-ui-2 rounded"
                                    >
                                        <MoreVertical class="w-4 h-4 text-light-tx-2 dark:text-dark-tx-2" />
                                    </button>
                    ...
                    {#if editMode && (ls.metadata?.eintraege || []).length === 0}
                        <tr class="border-b border-light-ui-3 dark:border-dark-ui-3">
                            ...
                            <td class="px-3 py-2"></td>
                            <td class="px-3 py-2 text-light-tx-3 dark:text-dark-tx-3 text-xs italic">
                                Keine Einträge
                            </td>
                            <td class="px-3 py-2"></td>
                            <td class="px-3 py-2 text-center">
                                <button
                                    onclick={() => addEintrag(kapIndex, lsIndex)}
                    ...
                    {:else if editMode}
                        <tr class="border-b border-light-ui-3 dark:border-dark-ui-3 bg-light-bg-2/50 dark:bg-dark-bg-2/50">
                ...
                {#if editMode && (kap.lernsequenzen || []).length === 0}
                    <tr class="bg-light-bg-2 dark:bg-dark-bg-2">
                        <td colspan="5" class="px-3 py-2 text-center text-sm text-light-tx-3 dark:text-dark-tx-3 italic">
                            Keine Lernsequenzen
                        </td>
                    </tr>
                    <tr class="bg-light-bg-2 dark:bg-dark-bg-2">
            ...
            {#if editMode && (curriculum?.kapitel || []).length === 0}
                <tr class="bg-light-bg-2 dark:bg-dark-bg-2">
                    <td colspan="5" class="px-3 py-4 text-center text-sm text-light-tx-3 dark:text-dark-tx-3">
